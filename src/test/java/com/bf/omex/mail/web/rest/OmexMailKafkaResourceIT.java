package com.bf.omex.mail.web.rest;
...
import com.bf.omex.mail.config.KafkaProperties;
...
class OmexMailKafkaResourceIT {
    ...
    void setup() {
        ...
        OmexMailKafkaResource kafkaResource = new OmexMailKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/omex-mail-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/omex-mail-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
